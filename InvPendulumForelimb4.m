...
arm_inches = 5.5;
arm = arm_inches/39.3701;
g = 9.81; %m/s

extended_l = 13 / 39.3701;
%extended_arm =  / 39.3701;
...
xup = zeros(length(theta), 1);
yup = zeros(length(theta), 1);
...
for i = 1:length(t)
    ...
    %Noodling for the green mid segment
    beta(i) = -th + phi;
    xup(i) = xp(i) + height*cos(beta(i));
    yup(i) = yp(i) + height*sin(beta(i));
 
    % Solve for intersection with x = 0
    s(i) = -xup(i) / cos(alpha(i));

    % Endpoint on vertical axis
    xc(i) = xup(i) + s(i)*cos(alpha(i));
    yc(i) = yup(i) + s(i)*sin(alpha(i));
    ...
    % Draw up haha
    plot([xp(i) xup(i)], [yp(i) yup(i)], ...
        'g', 'LineWidth', 3);

    % Draw to tree
    plot([xup(i) xc(i)], [yup(i) yc(i)], ...
        'r', 'LineWidth', 3);
...
forearm_extension = forearm_inches - 5.5;
